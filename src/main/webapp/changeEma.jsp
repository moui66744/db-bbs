...
<div >
    ...
    <div class="region w3-container w3-content w3-center w3-padding-64 w3-card w3-white " style="display:flex;align-items:center;max-width:800px;height:500px;margin-top: 40px;flex-direction: column">
        <h1 class="w3-wide" style="height: 100px;opacity: unset">修改邮箱</h1>
        ...
        <form id="loginForm" action="GetUserByNameAndPwdServlet.do" method="post" style="width: 400px;display: flex;align-items: center;flex-direction: column">
            <p style="align-self: flex-start">新邮箱</p>
            <input id="newEmail" class="w3-input" name="newEmail" style="max-width: 400px" type="text" placeholder="新邮箱" />

        </form>
        <div class="buttonItems" style="width: 500px;margin-top: 50px;opacity: unset">
            <button class="w3-button w3-black za-button" onclick=changeEmail()>确认修改</button>
        </div>
    </div>
    ...
    <script>
        function changeEmail(){
            $.ajax({
                type:"POST",
                url:"ChangeEmaByUserIdServlet.do",
                data:{
                    newEmail : $("#newEmail").val()
                },
                dataType:"text",
                success:function (res){
                    if (res === "1") window.location.href = "GetAllTopicServlet.do"
                    else alert("修改失败")
                }
            });
        }
